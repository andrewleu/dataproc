...
foreach code {"110000" "120000" "310000" "500000"} {
 ...
 #yidong avg
     ...
     set stat2 [mysqlsel $mysql_handler "select count(id),sum(avi) from download  where YM='$yandm'
         and agency = '移动' and addr like '$provience' " -list]
     set stat2 [string map {"{}" "0"} $stat2]
     set stat3 [mysqlsel $mysql_handler "select count(id),sum(avi) from download  where YM='$yandm'
         and agency = '移动' and addr like '$provience' " -list]
 ...
 while {$finish!=1}   {
  ...
  foreach code {710000  810000 820000} {
    ...
    set stat1 [string map {"{}" "0"} $stat1]
    set stat2 [mysqlsel $raw_data "select count(id), sum(avi) from download  where YM='$yandm' and addr like '$q_addr' " -list]
    set stat2 [string map {"{}" "0"} $stat2]
    set stat3 [mysqlsel $raw_data "select count(id), sum(avi) from download  where YM='$yandm' and addr like '$q_addr' " -list]
     set stat3 [string map {"{}" "0"} $stat3]
