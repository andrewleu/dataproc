...
set zipped 0 
#set files [glob sohu*]
while { [set firfile [lindex $flist $n_file]]!=""} {
   #set read_file "chinacache_20130501205026.txt" # to handle gziped file
    incr n_file;
    set read_file [lindex $flist $n_file]; puts $read_file
# if {[string match *gz $read_file]!= 0 } {
    exec cat $firfile $read_file >cc.gz
    exec gzip -dc cc.gz > $unzipfile
         ...
         if { [string length $msg] > 30} { 
           ...
           if { $hours >=19 && $hours < 23 }  {
            ...
            $hours >= 1 && $hours <6
              ...
              set msg "$msg$part1,$part2,$purpose,$status,$partition_col,$day,$read_file"
