...
set host {127.0.0.1}
set purpose "netcenter_201604*.new"
set user {ipv6bgp}
set password {ipv6}
set dbname {data}
set dltbl {dlfiles}
...
while { [set read_file [lindex $flist $n_file]]!=""} {
   ...
   set tempfilename $read_file
            #set tempfilename $read_file
   catch { exec gzip  -dc $read_file > $unzipfile} error
   if {$error!=""} {
       incr n_file;
       puts "not a gz file: $read_file"
       continue
   }
...
#   fconfigure $fp -encoding gb2312
   ...
   while {[eof $fp] != 1} {
           ...
           if { $hours >=19 && $hours < 23 }  {
            ...
            lappend msg ",netcenter,$status,$partition_col,$day,$tempfilename"
